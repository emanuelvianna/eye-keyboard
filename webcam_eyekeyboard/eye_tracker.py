...
class EyeTracker(object):
    ...
    def give_me_all(self, use_prect=False):
        ...
        thresholded = pg.surface.Surface(self.camera_size, 0, self.snapshot)
        th = (self.threshold, self.threshold, self.threshold)
        pg.transform.threshold(
            thresholded, self.snapshot, st.PUPIL_COLOUR, th, st.THRESHOLD_BACKGROUND_COLOUR, 1)

        self._find_pupil(thresholded, use_prect)
        return self.snapshot, thresholded
